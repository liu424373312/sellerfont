<template>
  <div id="dorReplenishList">
    <div class="weui-form-preview__ft" style="margin-top:10px;">
      <button class="weui-form-preview__btn weui-form-preview__btn_primary" v-if="this.replenishList.length > 0"
              style="background:#0bb20c;color:#fff;" @click="createMode">创建模板
      </button>
      <button class="weui-form-preview__btn weui-form-preview__btn_primary" v-if="this.goodsList.length > 0"
              @click="gotoList"
              style="border-top:2px solid rgba(7,17,27,0.3);border-bottom:2px solid rgba(7,17,27,0.3);border-right:2px solid rgba(7,17,27,0.3)">
        寝室上次配送单
      </button>
    </div>
    <div class="weui-panel__hd hd"><span class="weui-media-box__title" style="font-weight: bold">{{this.groupNo}}</span>&nbsp;配送单列表
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      createMode() {
        axios.get(API_PROXY + this.api + '/sell/seller/dispatch/create_template?token=' + this.token + '&groupNo=' + this.groupNo).then((res) => {
          console.log(res);
          if(res.data.msg === "成功"){
            weui.toast("创建模板成功!");
          }else{
