  ...
  <div class="goodMain">
      ...
      <div class="foods-wrapper" ref="foodsWrapper">
        <ul>
          <li v-for="(item,index) in goods" :key="index" class="food-list food-list-hook">
            ...
            <ul>
              <li v-if="food.quantity > 0" @click="selectFood(food,$event)" v-for="(food,index) in item.foods"
    ...
    <div class="weui-tabbar">
      <div class="weui-form-preview__ft" style="background:#0bb20c;width:100%;" @click="createRE">
        <button class="weui-form-preview__btn weui-form-preview__btn_primary"><span style="color:#fff;">创建配送单</span>
        </button>
      </div>
    </div>
  </div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created() {
      ...
      axios.get(API_PROXY + this.api + '/sell/seller/replenish/productlist?token=' + this.token + '&groupNo=' + this.groupNo).then((response) => {
        ...
        for (let i = 0; i < this.goods.length; i++) {
          for (let j = 0; j < this.goods[i].foods.length; j++) {
            if (this.goods[i].foods[j].count > 0) {
              this.amount += this.goods[i].foods[j].price * this.goods[i].foods[j].count;
            }
          }
        }
